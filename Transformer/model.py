...
class AccountAwareTransformer(nn.Module):
    ...
    def forward(self, acc_feat, txn_seq, mask=None):
        # acc_feat: [batch_size, account_feat_dim]
        # txn_seq: [batch_size, seq_len, txn_feat_dim]
        acc_token = self.acc_embedding(acc_feat).unsqueeze(1)  # [B, 1, D]
        txn_tokens = self.txn_embedding(txn_seq)               # [B, T, D]
        x = torch.cat([acc_token, txn_tokens], dim=1)          # [B, T+1, D]

        if mask is not None:
            # mask shape must be [B, T], need to pad for account token
            account_mask = torch.zeros((mask.shape[0], 1), dtype=torch.bool, device=mask.device)
            mask = torch.cat([account_mask, mask], dim=1)      # [B, T+1]

        x = self.transformer(x, src_key_padding_mask=mask)     # [B, T+1, D]
        cls_token = x[:, 0, :]  # use first token (account) as [CLS]
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for acc_feat, txn_seq, labels, mask in train_loader:
        acc_feat, txn_seq, labels = acc_feat.to(device), txn_seq.to(device), labels.to(device)
        mask = mask.to(device)

        optimizer.zero_grad()
        outputs = model(acc_feat, txn_seq, mask)
...
def evaluate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for acc_feat, txn_seq, labels, mask in val_loader:
            acc_feat, txn_seq, labels = acc_feat.to(device), txn_seq.to(device), labels.to(device)
            mask = mask.to(device)
